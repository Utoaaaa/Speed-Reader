...
async function generateArticle() {
    ...
    try {
        const response = await fetch('./api/deepseek', {
            ...
            body: JSON.stringify({
                messages: [{ role: "system", content: `請生成一篇繁體中文的故事性文章，長度約為 ${articleLength} 字。文章需包含完整事件脈絡、明確人物設定與具體細節，並穿插可用於閱讀理解測驗的資訊（如事件發生時間、地點、數據、因果關係、人物動機與行動結果等）。故事情節可以日常生活、校園事件、旅遊經歷、小型調查、職場互動或意外狀況為背景，避免寫成制式新聞或單純資料彙整報告。請務必包含至少兩個角色之間的互動，並讓事件具備起承轉合。文章結尾請提出一個封閉式的理解性問題，該問題必須對應內文中出現過的明確、可查證的事實細節，並且具有唯一正確答案。問題範例如：「林曉文最後是在哪裡找到她遺失的手機？」或「根據故事，阿凱是什麼時候決定離開公司的？」請避免是非題、主觀推論或感受性提問。請勿提供答案 ` }]
            }),
        });
